...
<body class="bg-gray-900 text-gray-300 min-h-screen p-2 md:ml-16 mb-10">
    ...
    <main>
        @if (session('success'))
            <div class="p-2">
                <div class="p-4 mb-4 text-sm rounded bg-green-200 text-gray-900 wrap-break-word md:text-nowrap"
                    role="alert">
    ...
    <div class="mb-2 flex md:justify-end">
        @if (auth()->user()->colocations()->where('colocation_id', $colocation->id)->first()?->pivot->role === 'owner')
            <form action="{{ route('coloc.cancel', $colocation) }}" method="post">
                @csrf
                <button type="submiy"
                    class="cursor-pointer rounded text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br font-medium rounded-base text-sm px-4 py-2.5 text-center leading-5 active:text-black">
                    Cancel Colocation
                </button>
            </form>
        @else
            <form action="{{ route('coloc.quit', $colocation) }}" method="post">
                @csrf
                <button
                    class="rounded-xl cursor-pointer inline-flex items-center justify-center p-0.5 font-medium bg-gradient-to-br from-red-200 via-red-300 to-yellow-200">
                    <span class="text-black px-4 py-2.5 leading-5">
                        Quit Colocation
                    </span>
                </button>
            </form>
    ...
    <div class="relative flex flex-col bg-white shadow-sm border border-slate-200 rounded-lg">
        <div class="p-4">
            <div class="mb-4 flex items-center justify-between">
                ...
                <p href="#" class="text-slate-600">
                    {{ $members }} members
                </p>
            </div>
            @forelse ($colocation->users()->wherePivot('left_at', null)->get() as $member)
                    ...
                    <div class="flex items-center justify-between pb-3 pt-3 last:pb-0">
                        <div class="flex items-center gap-x-3">
                            <p
                                ...
                                {{ Str::replace('...', '', Str::upper(Str::limit($member->name, 1))) }}</p>
                            <div>
    ...
    <div class="md:flex gap-3">
        {{-- invite --}}
        <div
            class="relative max-w-md w-full flex flex-col justify-center items-center my-6 bg-white border rounded-lg p-2">
