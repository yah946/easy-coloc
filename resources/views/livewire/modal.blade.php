        ...
        <div class="bg-black space-y-4 p-4 rounded-lg shadow-lg z-10 relative max-w-sm w-full mx-4">
            ...
            <div>
                <form wire:submit.prevent="create" method="post" class="space-y-4">
                    <div>
                        <input type="text" id="name" wire:model="name" placeholder="Name Of Colocation" class="rounded w-full bg-gray-700 px-2 py-1 outline-hidden focus:border-none focus:ring-1 focus:ring-yellow-200" required>
                        @error('name')
