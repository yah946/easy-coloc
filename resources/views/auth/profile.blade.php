...
<body class="bg-gray-900 text-gray-300 p-2 space-y-6 md:ml-16 mb-10">
    <main>
        ...
        <div class="border p-2 space-y-2">
            <h2 class="text-2xl font-bold m-0">{{ auth()->user()->name }}</h2>
            <p class="m-0">reputation: {{ auth()->user()->reputation }}</p>
            <p>{{ auth()->user()->email }}</p>
    ...
    <section id="edit_profile" class="hidden">
        <form method="post" class="flex flex-col gap-4 md:items-center">
            @csrf
            @method('put')
            <div class="max-w-md">
                <input value="{{ auth()->user()->name }}" name="name"
                    class="outline-hidden focus:ring-1 focus:border-none focus:ring-yellow-200 border w-full h-8 pl-4 pr-12"
        ...
        <form method="post" class="flex flex-col gap-4 md:items-center">
            ...
            <div class="max-w-md relative">
                <input name="current_password"
                    class="outline-hidden focus:border-none focus:ring-1 focus:ring-yellow-200 password-input border w-full h-8 pl-4 pr-12"
                    type="password" placeholder="Current Password">
                <button class="toggle-password absolute right-3 top-1/2 -translate-y-1/2">
                    <svg class="show-pass cursor-pointer w-6 h-6 text-white" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                        viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 6.2V5h11v1.2M8 5v14m-3 0h6m2-6.8V11h8v1.2M17 11v8m-1.5 0h3" />
                    </svg>
                    <svg class="hide-pass cursor-pointer hidden absolute right-0 top-1/2 -translate-y-1/2 w-6 h-6 text-white"
